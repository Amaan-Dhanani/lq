<script lang="ts">
	...
	let paths1 = '<path fill="" fill-rule="evenodd" d="M6.854 3.854a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L3.207 7.5zm6 0a.5.5 0 0 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L9.207 7.5z" clip-rule="evenodd"/>';
...
<div class="fixed left-0 right-0 -bottom-0 z-10 w-full">
	<div class="flex h-[118px] w-full flex-row items-end justify-around">
		...
		<!-- START AREAS TO CHANGE -->
		{#if number || number!=0}
		<Icon text="Previous" viewbox="0 0 15 15" paths={paths1} fill="lightcoral"/>
		{/if}
		<Icon text="Home" viewbox="0 0 48 48" paths={paths2} fill="lightcoral"/>
		{#if number || number!=0}
		<Icon text="Submit" viewbox="0 0 24 24" link="/create_a_quest/submit" paths={paths3} fill="lightcoral"/>
		{/if}
		{#if number!=10}
		<Icon text="Question {number + 1}" viewbox="0 0 15 15" paths={paths4} link="/create_a_quest/question{number +1}" fill="lightcoral"/>
		{/if}
		<!-- END AREAS TO CHANGE -->
	</div>
</div>
